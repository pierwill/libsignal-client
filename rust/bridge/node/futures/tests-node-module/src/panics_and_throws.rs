...
pub fn panic_pre_await(mut cx: FunctionContext) -> JsResult<JsObject> {
    ...
    signal_neon_futures::promise(&mut cx, |cx, future_context| {
        let future = future_context.clone().try_catch(cx, |cx| { JsFuture::new(cx, promise, future_context.clone(), move |cx, result| {
...
pub fn panic_during_callback(mut cx: FunctionContext) -> JsResult<JsObject> {
    ...
    signal_neon_futures::promise(&mut cx, |cx, future_context| {
        let future = future_context.clone().try_catch(cx, |cx| { JsFuture::new(cx, promise, future_context, |_cx, _result| {
            panic!("check for this")
            })});
        async move {
            future?.await;
...
pub fn throw_during_callback(mut cx: FunctionContext) -> JsResult<JsObject> {
    ...
    signal_neon_futures::promise(&mut cx, |cx, future_context| {
        let future = future_context.clone().try_catch(cx, |cx| { JsFuture::new(cx, promise, future_context.clone(), move |cx, _result| {
            future_context.try_catch(cx, |cx| {
